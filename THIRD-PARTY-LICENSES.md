...
Mani itself is MIT-licensed (see [LICENSE](LICENSE)).

## mpv

Mani drives [mpv](https://mpv.io/) as a separate subprocess over its
JSON IPC socket for audio playback - it's never linked into Mani's own
...
Mani redistributes itself, so nothing further is needed there. On
Windows, where there's no equivalent system package manager to depend on
mpv through, Mani bundles mpv's own binary directly (see
...
is only spawned as a subprocess rather than linked into Mani, this is
"mere aggregation" under the GPL - it doesn't require Mani's own source
