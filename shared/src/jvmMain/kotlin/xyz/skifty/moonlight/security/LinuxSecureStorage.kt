 ...
 *
 * Each secret is stored under the attribute pair `application=moonlight key=<key>`.
...
class LinuxSecureStorage : SecureStorage {
    ...
    init {
        if (!isSecretToolAvailable()) {
            throw IllegalStateException(
                "Moonlight needs the 'secret-tool' command to store credentials securely on " +
    ...
    override fun save(key: String, value: String) {
        val process = ProcessBuilder(
            "secret-tool", "store", "--label=Moonlight ($key)",
            "application", "moonlight", "key", key,
    ...
    override fun get(key: String): String? {
        val process = ProcessBuilder(
            "secret-tool", "lookup", "application", "moonlight", "key", key,
        ).redirectErrorStream(false)
            .start()
    ...
    override fun delete(key: String) {
        ...
        runCatching {
            ProcessBuilder(
                "secret-tool", "clear", "application", "moonlight", "key", key,
            ).start()
                .waitFor()
        }
    }
