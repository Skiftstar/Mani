 ...
 *  preview pinned to the bottom when there's an upcoming track. The cover art/title/details
 *  section scrolls independently (with the same auto-hiding scrollbar the main content area
 *  uses) so a long title/artist or extra song info never pushes the "Next in Queue" preview out
 *  of the panel entirely. Swapped out for `QueuePanel` in this same slot when the queue view is
 *  open - see [NowPlayingSidePanelShell], which both share. */
@Composable
fun NowPlayingPanel(
